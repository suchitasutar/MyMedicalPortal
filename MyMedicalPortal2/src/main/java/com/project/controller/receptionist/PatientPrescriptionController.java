...
public class PatientPrescriptionController 
{
	...
	public ModelAndView prescriptionQueue(String pid)
	{	
		try {
			...
			if(! prescriptionList.equals(null))
			{
				...
				mv.addObject("prescriptionsCount", dao.prescriptionPrintCount(pid));  //for receptionist only
				return mv;
			}
			else
			{   throw new Exception();  }
	...
	public ModelAndView delete(@RequestParam("pid")String pid)
	{
		try 
		{
			...
			if(i==1)
			{
				...
				int count=dao.prescriptionPrintCount(pid);
